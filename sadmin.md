...
:<% - все строки, 1,5, 1,& - от первой до последней><s - операция замены>/Строка/строка/<g - Global, поиск по всему файлу, а не по первому вхождению> c -запрашивать подтверждение или нет 

Варианты ответов для подтверждения:
  
Команда | Описание
---|---
y | Выполнит замену
n | Пропустить найденное значение
a | Выполнить все
q | Завершить
l | Выполнить текущее и завершить операцию
ctrl + e, ctrl + y | Прокрутить найденные значения вниз, вверх
/ | Поиск по всему файлу, повторить поиск командой n  

### Несколько файлов
```bash
vim test.txt vim.test2.txt;
```
Для переключения :N и :n (вперед).
Список файлов :buffers, затем номером выбрать нужный файл :buffer <номер>. Для того, чтобы открыть еще один файл набрать :e <имя файла>
  
### Вставка файла
:r <имя файла> - вставляет указанный файл перед курсором.  
